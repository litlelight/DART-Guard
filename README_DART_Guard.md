...
1. **Prepare** the forecasting models, expert set, reference policy, evidence rules, switching cost, and fault fallback before deployment.
2. **Proposal interval (15 days):** generate candidate forecasts and select one candidate using proposal-period performance only.
3. **Freeze:** lock the selected candidate and the fixed reference.
4. **Confirmation interval (15 days):** aggregate field-day evidence without using locked-query outcomes.
5. **Evidence gate:** release the candidate only when the predeclared evidence rule and switching-cost condition are both satisfied.
6. **Locked query (30 days):** evaluate once without retuning.
7. **Fault audit:** evaluate current-power loss, communication outage, missingness, and other sensor failures separately from Guard calibration.
...
1. the predeclared one-sided evidence screen is positive; and
2. the mean confirmation gain exceeds the chosen switching cost.
...
Email: `i24026647@student.newinti.edu.my`
